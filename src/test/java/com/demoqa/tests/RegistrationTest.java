...
//         $(byText(gender)).click(); // not very good (если будем проверять на разных языках)
        $("#genterWrapper").$(byText(gender)).click(); // самый лучший потому что читаемый
...
//         $(by("aria-label", "Choose Tuesday, March 13th, 1990")).click(); // не удобный, не универсальнвый

        $("#subjectsInput").setValue(subject).pressEnter();
        $("#hobbiesWrapper").$(byText(hobby1)).click();
        $("#hobbiesWrapper").$(byText(hobby2)).click();
        $("#uploadPicture").uploadFromClasspath("Zugpsitze_mountain.jpeg"); // короче
//         $("#uploadPicture").uploadFile(new File("src/test/resources/Zugpsitze_mountain.jpeg"));
